// When user bind any event like click, onChange, onblur all callback will have
// [e, obj, array, dataObjArray] arguments.
const formData = [
  {
    type: 'hidden',
    key: 'userId',
    ...
    getValue: function (obj) {
      // when user will use type hidden in formData object at that time we don't have to add any input
      // element in form but we should notify or give error to user in console if user has not mentioned
      // getValue function.
      // getValue function will only have current form data in form of object key value pair
      // first condition is to check if userId is present then use it because we only want to set userId
      // for new records
  ...
  },
  {
    ...
    label: 'Created At',
    getValue: function (obj) {
      // first condition is to check if createdAt is present then use it as we only want to set createdAt
  ...
  },
  {
    ...
    key: 'name',
    value: '',
    // attr is option, User can add new html element properties to it or user can remove all properties
    //like empty object  attr: {}
    attr: {
      id: 'txtName1',
